...
PVE 管理頁面默認可以通過這個地址`https://[本機本地 IP]:8006`訪問，例如`https://192.168.1.64:8006`。注意協議是`https`需要瀏覽器忽略一下自簽證書的安全檢查。管理頁面默認可以通過系統的`root`帳戶密碼登陸，`Realm` 選擇 `Linux PAM standard authentication`，你可以在進入系統之後配置其他登錄驗證方式。至此 PVE 節點已經完成了基本的安裝，下面我會開始介紹一些必須的配置項目。
